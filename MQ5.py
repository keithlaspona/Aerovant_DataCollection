#!/usr/bin/env python3
"""
AEROVANT: Real-Time Data Collection for MQ-5 Sensor
Collects and logs raw sensor data from a Raspberry Pi for the MQ-5 gas sensor.
"""
...
class DataCollector:
    """Collects real-time sensor data for the MQ-5 sensor."""
    def __init__(self):
        ...
        self.sensor_channel = {'MQ5_adc': AnalogIn(self.mcp, MCP.P2)}
        logger.info("Initialized hardware for MQ-5 sensor.")

    def _read_sensors(self) -> Dict[str, Any]:
        """Reads raw data from the MQ-5 and DHT sensors."""
    ...
    def collect_and_log_data(self, log_file: str, sample_interval: int = 10):
        ...
        logger.info(f"Starting MQ-5 data collection. Logging to {log_file}")
        ...
        while True:
            try:
                ...
                log_summary = f"MQ5_adc: {mq_reading}, Temp: {data_point['temp_c']}°C, Hum: {data_point['hum_pct']}%"
...
def main():
    parser = argparse.ArgumentParser(description="AEROVANT MQ-5 Data Collection")
    parser.add_argument("--output", type=str, default="mq5_sensor_data.csv", help="Output CSV file path.")
    parser.add_argument("--interval", type=int, default=10, help="Time interval between samples in seconds.")
    args = parser.parse_args()

    logger.info("AEROVANT MQ-5 Real-Time Data Collection Started")
    logger.info("=" * 50)
