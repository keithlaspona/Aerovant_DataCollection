#!/usr/bin/env python3
"""
AEROVANT: Real-Time Data Collection for MQ-4 Sensor
Collects and logs raw sensor data from a Raspberry Pi for the MQ-4 gas sensor.
"""
...
class DataCollector:
    """Collects real-time sensor data for the MQ-4 sensor."""
    def __init__(self):
    ...
    def _read_sensors(self) -> Dict[str, Any]:
        """Reads raw data from the MQ-4 and DHT sensors."""
    ...
    def collect_and_log_data(self, log_file: str, sample_interval: int = 10):
        ...
        logger.info(f"Starting MQ-4 data collection. Logging to {log_file}")
        ...
        while True:
            try:
                ...
                log_summary = f"MQ4_adc: {mq_reading}, Temp: {data_point['temp_c']}°C, Hum: {data_point['hum_pct']}%"
...
def main():
    parser = argparse.ArgumentParser(description="AEROVANT MQ-4 Data Collection")
    parser.add_argument("--output", type=str, default="mq4_sensor_data.csv", help="Output CSV file path.")
    parser.add_argument("--interval", type=int, default=10, help="Time interval between samples in seconds.")
    args = parser.parse_args()

    logger.info("AEROVANT MQ-4 Real-Time Data Collection Started")
    logger.info("=" * 50)
